  ...
  return (
    <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: '1.5rem', background: 'linear-gradient(145deg, #f7fbff 0%, #edf7ff 48%, #ffffff 100%)' }}>
      <section style={{ width: 'min(100%, 600px)', padding: 'clamp(2rem, 7vw, 4rem)', textAlign: 'center', borderRadius: '28px', background: '#fff', boxShadow: '0 24px 70px rgba(7, 53, 94, 0.12)', border: '1px solid #d7eafd' }}>
        ...
        <h1 style={{ margin: 0, color: '#081d3a', fontSize: 'clamp(1.8rem, 5vw, 2.7rem)', lineHeight: 1.12 }}>We are currently closed</h1>
        <p style={{ margin: '1.15rem auto 1.6rem', maxWidth: '430px', color: '#52647a', lineHeight: 1.7 }}>We are preparing today’s marketplace operations. Products, FINDit, orders, and seller tools reopen automatically at 10:00 Morocco time.</p>
        ...
        <p style={{ margin: '1.3rem 0 0', color: '#728197', fontSize: '.9rem' }}>Morocco time now: {formatMoroccoTime(now)} · {opensTomorrow ? 'Opening tomorrow at 10:00' : 'Opening today at 10:00'}</p>
      </section>
    </main>
  );
};
