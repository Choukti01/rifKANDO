  ...
  return (
    <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: '1.5rem', background: 'linear-gradient(145deg, #f7fbff 0%, #edf7ff 48%, #ffffff 100%)' }}>
      <section style={{ width: 'min(100%, 600px)', padding: 'clamp(2rem, 7vw, 4rem)', textAlign: 'center', borderRadius: '28px', background: '#fff', boxShadow: '0 24px 70px rgba(7, 53, 94, 0.12)', border: '1px solid #d7eafd' }}>
        ...
        <p style={{ margin: '1.15rem auto 1.6rem', maxWidth: '430px', color: '#52647a', lineHeight: 1.7 }}>{offline ? 'Our marketplace connection is temporarily unavailable. This page will reopen automatically when service returns.' : 'We are preparing today’s marketplace operations. Products, FINDit, orders, and seller tools reopen automatically at 10:00 Nador time.'}</p>
        <div style={{ padding: '1rem 1.25rem', borderRadius: '14px', background: '#f5faff', color: '#163a60', fontWeight: 700 }}>Open every day · 10:00–22:00 <span style={{ color: '#1684d8' }}>Nador · UTC</span></div>
        <p style={{ margin: '1.3rem 0 0', color: '#728197', fontSize: '.9rem' }}>Nador time now: {formatMoroccoTime(now)}{offline ? ' · Checking the connection automatically' : ` · ${opensTomorrow ? 'Opening tomorrow at 10:00' : 'Opening today at 10:00'}`}</p>
      </section>
    </main>
  );
};
